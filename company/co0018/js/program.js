...
function confirmationProcess() {
  ...
  // お客様番号
  var customerNumberElement = document.getElementById("customerNumber");
  sessionStorage.setItem(
    "customerNumberValue",
    customerNumberElement.customerNumber.value
  );
  // 問い合わせ内容
...
function getSessionData() {
  ...
  // お客様番号
  var customerNumberElement = document.getElementById("customerNumber");
  for (let i = 0; i < customerNumberElement.customerNumber.length; i++) {
    if (
      customerNumberElement.customerNumber[i].value ==
      sessionStorage.getItem("customerNumberValue")
    ) {
      customerNumberElement.customerNumber[i].checked = true;
    } else {
      customerNumberElement.customerNumber[i].disabled = true;
    }
  }
