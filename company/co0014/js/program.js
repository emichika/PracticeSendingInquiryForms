...
function confirmationProcess() {
  ...
  // セッションデータを保存
  var element = document.getElementById("inquiryItems");
  sessionStorage.setItem("inquiryItemsValue", element.inquiryItems.value);
  // 会社名
  ...
  // ご利用店舗名
  sessionStorage.setItem("storeNameValue", storeName.value);
  // 問い合わせ内容
...
function sendLoad() {
  ...
  // ご利用店舗名
  document.getElementById("storeNameResult").innerText =
    "専門フォームの項目のため、送信対象外となります。";
  document.getElementById("storeNameResult").style.color = "#FF0000";
  // 問い合わせ内容
...
function endProcess() {
  ...
  sessionStorage.removeItem("storeNameValue");
...
function getSessionData() {
  // お問い合わせ項目
  var element = document.getElementById("inquiryItems");
  for (let i = 0; i < element.inquiryItems.length; i++) {
    if (
      element.inquiryItems[i].value ==
      sessionStorage.getItem("inquiryItemsValue")
    ) {
      element.inquiryItems[i].checked = true;
    } else {
      element.inquiryItems[i].disabled = true;
    }
  }
  ...
  // ご利用店舗名
  document.getElementById("storeName").value = sessionStorage.getItem(
    "storeNameValue"
  );
  // 問い合わせ内容
